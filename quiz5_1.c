...
void init()
{
    ...
    config_dom = domain_new( (void (*)(void *)) delete_config);
    if (!config_dom)
...
{
    (void) arg;
    struct timespec t1 = {.tv_sec = 0, .tv_nsec = 200};
...
static void *cleanup_thread(void *arg){
    bool *stop = (bool *) arg;
    struct timespec t1 = {.tv_sec = 0, .tv_nsec = 300};
    while(!(*stop)){
        printf("cleanup\n");
...
int main()
{
    ...
    bool stop = false;
    if (pthread_create(&cleaner, NULL, cleanup_thread, &stop))
        warn("pthread_create");
